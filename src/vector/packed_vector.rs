...
#[derive(Debug, Clone)]
pub struct PackedVector<Item: Serialize, const PACKING: usize = 1> {
    layers: Vec<Vec<Hash>>,
    items: Vec<Item>,
}

impl<Item, const PACKING: usize> PackedVector<Item, PACKING>
where
    Item: Serialize,
{
...
impl<Item, const PACKING: usize> Serialize for PackedVector<Item, PACKING>
where
    Item: Serialize,
{
...
impl<'de, Item, const PACKING: usize> Deserialize<'de> for PackedVector<Item, PACKING>
where
    Item: Serialize + Deserialize<'de> + Clone,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(PackedVector::new(items).map_err(|err| DeError::custom(err))?)
    }
}
...
mod tests {
    ...
    #[should_panic]
    fn empty() {
        PackedVector::<()>::new(vec![]).unwrap();
    }

    #[test]
    fn one_item() {
        let vector = PackedVector::<_>::new(vec![0u32]).unwrap();
    ...
    #[test]
    fn one_item_2packed() {
        let vector = PackedVector::<_, 2>::new(vec![0u32]).unwrap();
    ...
    #[test]
    fn one_item_3packed() {
        let vector = PackedVector::<_, 3>::new(vec![0u32]).unwrap();
    ...
    #[test]
    fn two_items() {
        let vector = PackedVector::<_>::new(vec![0u32, 1u32]).unwrap();
    ...
    fn two_items_2packed() {
        let vector = PackedVector::<_, 2>::new(vec![0u32, 1u32]).unwrap();
    ...
    #[test]
    fn three_items() {
        let vector = PackedVector::<_>::new(vec![0u32, 1u32, 2u32]).unwrap();
    ...
    fn three_items_2packed() {
        let vector = PackedVector::<_, 2>::new(vec![0u32, 1u32, 2u32]).unwrap();
    ...
    fn three_items_3packed() {
        let vector = PackedVector::<_, 3>::new(vec![0u32, 1u32, 2u32]).unwrap();
    ...
    fn proof_stress() {
        for len in 1..128 {
            let vector = PackedVector::<_>::new((0..len).collect()).unwrap();
    ...
    fn proof_stress_2packed() {
        for len in 1..128 {
            let vector = PackedVector::<_, 2>::new((0..len).collect()).unwrap();
    ...
    fn proof_stress_3packed() {
        for len in 1..128 {
            let vector = PackedVector::<_, 3>::new((0..len).collect()).unwrap();
    ...
    fn proof_stress_4packed() {
        for len in 1..128 {
            let vector = PackedVector::<_, 4>::new((0..len).collect()).unwrap();
    ...
    #[test]
    fn serde() {
        let original = PackedVector::<_>::new((0..128).collect()).unwrap();
        let serialized = bincode::serialize(&original).unwrap();
        let deserialized = bincode::deserialize::<PackedVector<u32>>(&serialized).unwrap();
    ...
    #[test]
    fn serde_2packed() {
        let original = PackedVector::<_, 2>::new((0..128).collect()).unwrap();
        let serialized = bincode::serialize(&original).unwrap();
        let deserialized = bincode::deserialize::<PackedVector<u32, 2>>(&serialized).unwrap();
    ...
    #[test]
    fn serde_3packed() {
        let original = PackedVector::<_, 3>::new((0..128).collect()).unwrap();
        let serialized = bincode::serialize(&original).unwrap();
        let deserialized = bincode::deserialize::<PackedVector<u32, 3>>(&serialized).unwrap();
